...
/*
    //testing if category variable matches category in vector
    string cat;
    cout << "Which category would you like to browse? Please type in category exactly as listed!" << endl; 
    cin >> cat;

    cout<< cat<<endl;

    cout << category[0].getCat()<<endl;

    if(cat == category[0].getCat()){
        cout << "true" <<endl;
    }
*/
    ...
    //loop through category vector and see if user input is there
    ...
    //cout<< BI_choice<<endl; testing what BI_choice is
...
//ADD TO LIBRARY FUNCTION
    if(BI_choice == 1){
        ...
        //implement a way to verify user's input is only one word

        insert_txt_name = insert_txt_name + ".txt";
        
        //will need to implement a way to check if the txt name already exists in either files
        ...
        while(check==true){
            ...
            check = false;
            //unable to fix potential error
            cout << "Give the file of your peom a name (No spaces as this will mess up the program!). This should be different from the poem's name: "<<endl;
            cin >> insert_txt_name;
...
//BROWSE FUNCTION
    ...
    //testing what is put into poem_file
    //cout << poem_file <<endl;
    

    //testing out if the linked lists has the files
    //Happy.printList();
    //Sad.printList();


    //add a function that randomly suggests a poem????
return 0;
}